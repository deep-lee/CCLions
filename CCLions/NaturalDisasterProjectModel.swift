...
let NATURAL_PROJECT_GET_DATAARRAY_FROM_DISK_CACHE_SUCCESS  = "NATURAL_PROJECT_GET_DATAARRAY_FROM_DISK_CACHE_SUCCESS"
...
class NaturalDisasterProjectModel: SuperModel {
    ...
    func updateDiskCache() -> Void {
        let cache = YYCache(name: NATURAL_PROJECT_CACHE)
        cache?.diskCache.setObject(self.dataArray, forKey: NATURAL_PROJECT_DATAARRAY)
    }
    ...
    func getDiskCache() -> Void {
        let cache = YYCache(name: NATURAL_PROJECT_CACHE)
        let array = cache?.diskCache.objectForKey(NATURAL_PROJECT_DATAARRAY) as? [Project]
        if array != nil {
            print(array?.count)
            self.dataArray = array
            self.postNotification(NATURAL_PROJECT_GET_DATAARRAY_FROM_DISK_CACHE_SUCCESS)
        } else {
            self.postNotification(NATURAL_DISASTER_PROJECT_REFRESH_DATA_FINISH)
        }
    }
