...
- (void)setPassedCount:(CGFloat)passedCount {
    _passedCount = passedCount;
    _initialProgress = [self calculatePercent:_passedCount toCount:_totalCount];
...
- (double)percent {
    ...
    _percent = [self calculatePercent:_passedCount toCount:_totalCount];
    return _percent;
}
...
- (double)calculatePercent:(CGFloat)fromCount toCount:(CGFloat)toCount {
    
    if ((toCount > 0) && (fromCount > 0)) {
        
        CGFloat progress = 0;
        
        progress = fromCount / toCount;
