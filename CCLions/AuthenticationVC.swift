...
class AuthenticationVC: UIViewController {
     ...
     */
    func initNoti() -> Void {
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AuthenticationVC.addAuthenticationSuccessCallBack(_:)), name: ADD_AUTHENTICATION_SUCCESS, object: nil)
    }
    ...
    func getImageArray() -> [UIImage] {
        ...
        for index in 0 ..< dataArray.count {
            ...
            let cell = self.mTableView.cellForRowAtIndexPath(indexPath) as! AuthenticationTableViewCell
            array.append(cell.imageViewShow.image!)
        }
    ...
    func complete() -> Void {
        ...
        // 发起申请
        AuthenticationModel.shareInstance().requestAuthentication(self.getImageArray())
    }
    ...
    /**
     ...
     - parameter noti: 通知
     */
    func addAuthenticationSuccessCallBack(noti: NSNotification) -> Void {
        // 修改本地用户当前认证状态
        // AuthenticationModel.shareInstance().updateLoginedUserInfo()
