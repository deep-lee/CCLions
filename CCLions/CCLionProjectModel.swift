...
let CCLION_PROJECT_GET_DATAARRAY_FROM_DISK_CACHE_SUCCESS = "CCLION_PROJECT_GET_DATAARRAY_FROM_DISK_CACHE_SUCCESS"
...
class CCLionProjectModel: SuperModel {
    ...
    func updateDiskCache() -> Void {
        let cache = YYCache(name: CCLION_PROJECT_CACHE)
        cache?.diskCache.setObject(self.dataArray, forKey: CCLION_PROJECT_DATAARRAY)
    }
    ...
    func getDiskCache() -> Void {
        let cache = YYCache(name: CCLION_PROJECT_CACHE)
        let array = cache?.diskCache.objectForKey(CCLION_PROJECT_DATAARRAY) as? [Project]
        if array != nil {
            print(array?.count)
            self.dataArray = array
            self.postNotification(CCLION_PROJECT_GET_DATAARRAY_FROM_DISK_CACHE_SUCCESS)
        } else {
            self.postNotification(CCLION_PROJECT_REFRESH_DATA_FINISH)
        }
    }
